...
import { createTray, registerGlobalShortcuts, unregisterGlobalShortcuts } from './tray';
import { createBubbleWindow, registerBubbleIPC, destroyBubbleWindow } from './window/floatBubble';
...
import { optimizationConfig } from './services/optimizationConfig';
import { registerPerformanceRoutes } from './ipcRoutes/performance';
import { registerVisualizationRoutes } from './ipcRoutes/visualization';
...
const initializeApp = async () => {
  ...
  // 初始化优化功能（默认开启低风险功能，可按需调整）
  optimizationConfig.enableSafe();
  logger.info('[optimization] flags:', optimizationConfig.getFlags());
...
const registerIPCHandlers = (
  ...
  // 注册优化功能 IPC 路由
  if (optimizationConfig.isEnabled('enablePerformanceMonitor')) {
    registerPerformanceRoutes();
  }
  if (optimizationConfig.isEnabled('enableVisualization')) {
    registerVisualizationRoutes();
  }
