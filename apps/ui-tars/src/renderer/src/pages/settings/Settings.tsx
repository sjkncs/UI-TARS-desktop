...
import { RefreshCcw, Trash, Keyboard } from 'lucide-react';
...
const SHORTCUT_LIST = [
  { key: 'shortcuts.showHide' as const, shortcut: 'Ctrl+Shift+T' },
  { key: 'shortcuts.newChat' as const, shortcut: 'Ctrl+Shift+N' },
  { key: 'shortcuts.openSettings' as const, shortcut: 'Ctrl+Shift+,' },
];
              ...
              >
                ...
                <div className="flex items-center gap-3 pt-2">
                  <Button
                    ...
                    onClick={async (e) => {
                      e.preventDefault();
                      const baseUrl = form.getValues('vlmBaseUrl');
                      const apiKey = form.getValues('vlmApiKey');
                      if (!baseUrl || !apiKey) {
                        setApiTestResult({ status: 'not_configured', message: t('settings.apiNotConfigured') });
                        return;
                      }
                      setApiTestLoading(true);
                      setApiTestResult(null);
                      try {
                        const res = await window.electron.ipcRenderer.invoke('system:checkApiStatus');
                        setApiTestResult({
                          status: res.status,
                          message: res.status === 'connected'
                            ? t('settings.apiConnected')
                            : `${t('settings.apiDisconnected')}: ${res.message}`,
                        });
                      } catch (err) {
                        setApiTestResult({
                          status: 'error',
                          message: `${t('settings.apiDisconnected')}: ${err instanceof Error ? err.message : 'Unknown'}`,
                        });
                      } finally {
                  ...
                  >
                    {apiTestLoading ? t('settings.testing') : t('settings.testApi')}
                  </Button>
                  {apiTestResult && (
                    <span className={`text-sm ${
                      apiTestResult.status === 'connected' ? 'text-green-600' :
                      apiTestResult.status === 'not_configured' ? 'text-amber-600' : 'text-red-600'
                    }`}>
                  ...
                  {SHORTCUT_LIST.map((item, idx) => (
                    <div
                      ...
                      className={`flex items-center justify-between px-4 py-3 ${
                        idx < SHORTCUT_LIST.length - 1 ? 'border-b border-gray-100 dark:border-gray-800' : ''
                      }`}
                    >
                      <span className="text-sm">{t(item.key)}</span>
                      <kbd className="inline-flex items-center gap-1 px-2 py-1 text-xs font-mono font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded border border-gray-300 dark:border-gray-600">
                        {item.shortcut}
                      </kbd>
                    </div>
                  ))}
