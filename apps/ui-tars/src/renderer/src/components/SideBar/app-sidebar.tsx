...
export function AppSidebar({ ...props }: ComponentProps<typeof Sidebar>) {
  ...
  const onSessionDelete = useCallback(
    async (sessionId: string) => {
      ...
      // Clean up chat messages stored in localStorage
      localStorage.removeItem('ui-tars-chat-' + sessionId);
      if (localStorage.getItem('ui-tars-chat-current-session') === sessionId) {
    ...
    <>
      <Sidebar collapsible="icon" className="select-none" {...props}>
        ...
        <SidebarFooter className="p-0">
          <NavToolbar onDashboardClick={handleDashboardClick} />
