...
const getIcon = (operator: Operator, isActive: boolean) => {
  ...
  return (
    <div className="relative flex items-center gap-1">
      ...
      <div
        className={`absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full text-[6px] flex items-center justify-center font-bold leading-none bg-background border border-gray-500 dark:border-gray-400 ${isActive ? 'text-neutral-700 border-neutral-700' : 'text-neutral-500 border-neutral-500'}`}
      >
        {isRemote ? 'R' : 'L'}
      </div>
    </div>
                ...
                <SidebarMenuSub className="!mr-0 !pr-1">
                  {history.map((item) => (
                    <SidebarMenuSubItem key={item.id} className="group/item">
                      <SidebarMenuSubButton
                        className={`hover:bg-neutral-100 hover:text-neutral-600 py-5 cursor-pointer ${item.id === currentSessionId ? 'text-neutral-700 bg-accent hover:bg-accent dark:text-neutral-200' : 'text-neutral-500'}`}
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                          <span className="max-w-38 truncate text-sm">{item.name}</span>
                          <span className="text-[10px] text-muted-foreground">{formatRelativeTime(item.updatedAt)}</span>
                        </div>
                      </SidebarMenuSubButton>
                      <DropdownMenu>
