      ...
      {showExport && (
        <ExportDialog
          ...
          messages={chatMessages.map((msg) => {
            ...
            // For assistant messages, extract the finished step content if available
            if (msg.from === 'gpt' && msg.predictionParsed?.length) {
              const finishedStep = msg.predictionParsed.find(
                (step) =>
                  step.action_type === 'finished' &&
                  step.action_inputs?.content &&
                  typeof step.action_inputs.content === 'string',
              ...
              if (finishedStep?.action_inputs?.content) {
                content = finishedStep.action_inputs.content;
              }
            }
            return {
              role: msg.from === 'human' ? 'user' as const : 'assistant' as const,
              content,
              screenshotBase64: msg.screenshotBase64WithElementMarker || msg.screenshotBase64 || undefined,
              timestamp: msg.timing?.start,
            };
          })}
        />
      )}
